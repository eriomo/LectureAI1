...
WHAT TO SAY:\n[2-3 sentences]\nMISCONCEPTION:\n[1 sentence]\nFOLLOW-UP:\n[1 question]\nQUICK ACTIVITY:\n[2 min activity]"""
    try:
        return jsonify({"result": ask_groq_text(prompt)})
    except Exception as e:
...
Personalise to their background. Everyday analogies. Under 200 words. End with encouragement using their name. No headers or bullets."""
    try:
        return jsonify({"result": ask_groq_text(prompt)})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def generate_slides():
    ...
    for i, obj in enumerate(obj_lines[:6]):
        y = 1.5 + i * 0.85
        add_rect(s2, 0.5, y, 0.4, 0.55, GREEN)
    ...
    add_rect(sc, 0, 0, 13.33, 7.5, GREEN)
    add_rect(sc, 0, 5.5, 13.33, 2.0, DARKGREEN)
